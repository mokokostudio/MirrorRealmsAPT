    ...
    public class TcpProtoHandle {
        ...
        private async Task<object> Deserialize(int len, int msg) {
            ...
            using (var ms = new MemoryStream(buffer, 0, len))
                return null; //ProtoBuf.Serializer.Deserialize(s_Msg2TypeDic[msg], ms);
        }
        ...
        private void WriteToStream<T>(T obj) {
            ...
            WriteNumber(msg);
            WriteNumber(verify);
            //ProtoBuf.Serializer.SerializeWithLengthPrefix(m_Stream, obj, ProtoBuf.PrefixStyle.Fixed32);
            m_Stream.Flush();
        }
