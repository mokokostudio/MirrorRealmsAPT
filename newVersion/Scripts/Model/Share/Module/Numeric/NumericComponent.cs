...
namespace ET
{
    ...
    public static class NumericComponentSystem
    {
        ...
        public static void Update(this NumericComponent self, int numericType, bool isPublicEvent)
        {
            ...
            var baseV = self.GetByKey(bas);
            var addV = self.GetByKey(add);
            var pctV = self.GetAsFloat(pct);
            var finalAddV = self.GetByKey(finalAdd);
            var finalPctV = self.GetAsFloat(finalPct);
            var result = (long)(((baseV + addV) * (100 + pctV) / 100f + finalAddV) * (100 + finalPctV) / 100f);
